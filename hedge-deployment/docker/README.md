...
1.	Start docker: `sudo systemctl start docker` or `sudo service docker start`
   

#### b.	Configure the size of log-files (optional):
...
#### c. Login to docker registry hub.docker.com (Optional)
...
#### d. Check if whiptail is installed.
...
#### f. Install the Make package as per your OS
```
sudo apt update
...
The registry password is required to seed it during installation, but we can remove it after the installation.
...
If using the package installer, you can provide the admin user & password & install directly from hedge-deployment/docker
The default username & password are admin/admin.
It is recommended to change this password for the production environment.
...
Additionally, nats server establishes a secure WebSocket-based connection, so the following files that are generated 
from the installation step on the core server are to be copied to the node server before installation can start
Here is the list of files on the core server that need to be copied to the node server. On both servers, the files are in /tmp folder.
