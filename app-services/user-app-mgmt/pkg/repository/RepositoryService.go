...
import (
	...
	_ "github.com/lib/pq"
	"gorm.io/gorm"
	"hedge/app-services/user-app-mgmt/pkg/config"
	"hedge/app-services/user-app-mgmt/pkg/dto"
	"os"
	"os/exec"
	"strings"
	"time"
...
func (r *UserManagementRepository) InitializeHedgeDB() error {
	...
	dwpUrl, err := r.appService.GetAppSetting("Digital_Workplace_Url")
	if err != nil {
		r.appService.LoggingClient().Warnf("Error while getting app setting: %v", err)
	}
	helixUrl, err := r.appService.GetAppSetting("Helix_Monitor_Url")
	if err != nil {
		r.appService.LoggingClient().Warnf("Error while getting app setting: %v", err)
	}
	...
	sqldata := strings.Replace(string(data), "DWP_URL", dwpUrl, -1)
	sqldata = strings.Replace(sqldata, "HELIX_URL", helixUrl, -1)
	sqldata = strings.Replace(sqldata, "USERNAME", username, -1)
	...
	if !dbExists {
		...
		// if profile=demo/all during startup, then apply the demo menus SQL script
		prof := os.Getenv("COMPOSE_PROFILES")
		if strings.Contains(prof, "all") || strings.Contains(prof, "demo") {
			data, err := os.ReadFile(dbDemoMenusFile)
			if err != nil {
				r.appService.LoggingClient().Errorf("Error while reading the DB demo-menus file (%s): %v", dbDemoMenusFile, err)
				return err
			}

			ret := db.Exec(string(data))
			if ret.Error != nil {
				r.appService.LoggingClient().Errorf("DB demo-menus Init failed: %v", ret.Error)
				return ret.Error
			}
			r.appService.LoggingClient().Info("DB demo menus created successfully")
		}
	} else {
